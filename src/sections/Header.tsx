...
const Header = ({ scrollY }: HeaderProps) => {
      ...
      megaMenu: [
        {
          category: 'Gros Œuvre',
          links: [
            { name: 'Presentation', href: '/docs/hormexpres-construction-doc.pdf', isDownload: true },
  ...
  const scrollToSection = (hrefOrPath: string | undefined) => {
    if (!hrefOrPath) return;
    
    // Check if it's a route path (starts with / but not /#)
    if (hrefOrPath.startsWith('/') && !hrefOrPath.includes('#')) {
      navigate(hrefOrPath as string);
      setIsMenuOpen(false);
      setIsServicesOpen(false);
      return;
    }
    ...
    // Handle hash links
    const [path, hash] = hrefOrPath.split('#');
    if (hash) {
      ...
      if (path && path !== '/') {
        ...
        setTimeout(() => {
          ...
          if (element) {
            (element as Element).scrollIntoView({ behavior: 'smooth' as ScrollBehavior });
          }
        }, 100);
      } else {
        const element = document.querySelector(`#${hash}`);
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              ...
              >
                ...
                {link.megaMenu && (
                  <div
                    className={`absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[1000px] max-w-[94vw] bg-white shadow-2xl rounded-xl overflow-hidden border border-gray-100 transition-all duration-300 ${
                      isServicesOpen
                              ...
                              <li key={item.name}>
                                {item.isDownload ? (
                                  <a 
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center gap-2 w-full text-left text-sm text-gray-600 hover:text-[#fbab39] hover:translate-x-1 transition-all duration-200 font-medium"
                                  >
                                    {item.name}
                                    <Download size={14} />
                                  </a>
